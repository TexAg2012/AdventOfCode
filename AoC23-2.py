...
Data = """Game 1: 2 green, 6 blue, 7 red; 12 green, 6 blue, 3 red; 5 red, 18 green, 4 blue
...
DataSmall = """Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
for d in DataSplit:
    game = re.findall("Game (\d+):",d)[0]

    if max(map(int,re.findall(" (\d+) red",d))) <= 12 and max(map(int,re.findall(" (\d+) green",d))) <= 13 and max(map(int,re.findall(" (\d+) blue",d))) <= 14:
        runTotal += int(game)

print("Part one answer: ", runTotal)
...
for d in DataSplit:
    ...
    r = max(np.asarray(re.findall(" (\d+) red",d)).astype(int))
    b = max(np.asarray(re.findall(" (\d+) green",d)).astype(int))
    g = max(np.asarray(re.findall(" (\d+) blue",d)).astype(int))

    print(r,b,g,r*b*g)

    sumPower += r*b*g

print("Part two answer: ", sumPower)
