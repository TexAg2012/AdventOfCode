...
for d in dataSplit:
    digits.append(re.findall(r'\d',d))

total = 0

for d in digits:
    #Add the farthest left [0] to farthest right [-1]
    total += int(d[0]+d[-1])
...
comboNum = ['zerone','oneight','twone','threeight','fiveight','sevenine','eightwo','nineight']
comboRes = ['01','18','21','38','58','79','82','98']
textNum = ['zero','one','two','three','four','five','six','seven','eight','nine']

for i in range(len(comboNum)):
    data = data.replace(comboNum[i],comboRes[i])

for i in range(10):
    data = data.replace(textNum[i],str(i))
