...
Data = """F|J-F|.|.F-F-7-7F--|77F.7.77F|-L7-|7|-|-F-L7.7FJF-7-JF7.77-F.7F7.FLF7-FF-L7F--F-J-..J-FJ-FLF7.FF.-F7-|-.F7--77F---7-LF7FFL7.F7|7FJ-FL-.-FJ-F
...
#Moves [row,column] index based on pipe depending on NSEW incoming direction
#7W represents a 7 where the previous point was from the west of current element and moves it one row higher and no columns over
Directions = {'7W':[1,0],'7S':[0,-1],
              'JW':[-1,0],'JN':[0,-1],
              'LN':[0,1],'LE':[-1,0],
              'FS':[0,1],'FE':[1,0],
              '|N':[1,0],'|S':[-1,0],
...
print(np.where(DataMatrix=='S'))
print(np.array([1,0]))
print(DataMatrix[np.where(DataMatrix=='S')])

#Check all four directions for linkable pipe
